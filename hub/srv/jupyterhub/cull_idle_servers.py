    ...
    if protected_users is not None:
        ...
        for protected in protected_users.split(","):
            if os.path.exists(protected):
                db_path = protected
                try:
                    app_log.info("Cull, checking db {} " "for users".format(db_path))
                    file_users = []
                    with open(db_path, "r") as db:
                        file_users = [user.rstrip("\n").rstrip("\r\n") for user in db]
                    p_users.extend(file_users)
                except IOError as err:
                    app_log.error(
                        "Cull, tried to open db file {},"
...
if __name__ == "__main__":
    ...
    define(
        "protected_users",
        default=None,
        help="List of users that shouldn't " "have their servers killed",
    )
    define(
        "max_age",
        default=0,
