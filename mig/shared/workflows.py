...
def __refresh_map(configuration, to_refresh):
    ...
    workflow_map, map_stamp = __load_map(
        configuration, to_refresh, do_lock=False)
...
def __update_workflow_pattern(configuration, client_id, vgrid,
    ...
    pattern = get_wp_with(
        configuration, client_id=client_id, first=True, owner=client_id,
        persistence_id=persistence_id, vgrids=vgrid)
    ...
    _logger.debug('update_workflow_pattern, resulting pattern: '
                  + str(pattern))
...
def __rule_identification_from_pattern(configuration, client_id,
    ...
    logger.info('%s is identifying any possible tasks from pattern creation '
    ...
    for recipe_name in workflow_pattern['recipes']:
        logger.info("looking for recipe :" + recipe_name)

        recipe = get_wr_with(
            configuration, client_id=client_id, first=True, name=recipe_name,
            vgrids=vgrid)
        logger.info("recipe :" + str(recipe))

        if recipe:
            recipe_list.append(recipe)
            logger.info("found and adding recipe :" + recipe_name)
        else:
...
def __rule_identification_from_recipe(configuration, client_id,
    ...
    if not patterns:
        return False, 'No appropriate patterns to check for recipe %s' \
    ...
    for pattern in matching_patterns:
        ...
        for recipe_name in pattern['recipes']:
            recipe = get_wr_with(
                configuration, client_id=client_id, name=recipe_name,
                vgrids=vgrid)
            if recipe:
                recipe_list.append(recipe)
            else:
        ...
        if status:
            ...
            for trigger in trigger_list:
                if trigger['rule_id'] == trigger_id:
                    ...
                    if 'pattern' in trigger.keys():
                        ...
                        __update_workflow_pattern(
                            configuration, client_id, vgrid,
                            new_pattern_variables, pattern_id)
            ...
            for recipe_name in recipe_list:
                recipe = get_wr_with(
                    ...
                    persistence_id=recipe_name, vgrids=vgrid)
                new_recipe_variables = {
                    'triggers': recipe['triggers']
                }
                new_recipe_variables['triggers'].pop(
...
def create_single_input_trigger(configuration, _logger, vgrid, client_id,
    ...
    __update_workflow_pattern(
        configuration, client_id, vgrid, new_pattern_variables,
    ...
    if 'name' not in pattern:
        ...
        if existing_pattern:
            ...
            status, msg = __update_workflow_pattern(
                configuration, client_id, vgrid, pattern, persistence_id)
    ...
    if 'name' not in recipe:
        ...
        if existing_recipe:
            ...
            status, msg = __update_workflow_recipe(
                configuration, client_id, vgrid, recipe, persistence_id)
